...
def evaluate_ae_mae(dataloader, encoder, decoder, device, save_dir, data_split):
    encoder.eval()
    decoder.eval()
    mae_scores, true_labels = [], []
    saved_images = False
            ...
            if not saved_images:
                ...
                if has_bg and has_anom:
                    save_reconstruction_pairs_by_class(
                        ...
                        save_dir=save_dir, data_split=data_split, model_name="AE_MAE", num_per_class=3
                    )
                    saved_images = True

            # Anomaly Score basato sulla MAE (L1 Loss) per singola immagine
            mae_per_pixel = F.l1_loss(reconstructed, batch_x, reduction='none')
            mae_per_image = mae_per_pixel.view(mae_per_pixel.size(0), -1).mean(dim=1)
            
            mae_scores.extend(mae_per_image.cpu().numpy())
            true_labels.extend(batch_y.numpy())

    mae_scores = np.array(mae_scores)
    true_labels = np.array(true_labels)
    mean_loss = np.mean(mae_scores)
    print(f"\nResults {data_split.upper()}:")
    print(f"Mean Reconstruction Loss (MAE): {mean_loss:.6f}")
    
    if np.sum(true_labels == 1) > 0:
        plt.figure(figsize=(10, 6))
        sns.histplot(mae_scores[true_labels == 0], color='blue', label='Background', kde=True, stat='density', alpha=0.5, bins=50)
        sns.histplot(mae_scores[true_labels == 1], color='red', label='Anomalies', kde=True, stat='density', alpha=0.5, bins=50)
        plt.xlabel('Reconstruction Error (MAE)')
        plt.ylabel('Density')
        plt.title(f'AE MAE Score Dist - {data_split.capitalize()}')
        plt.legend()
        plt.savefig(os.path.join(save_dir, f'loss_dist_ae_mae_{data_split}.png'), bbox_inches='tight')
        plt.close()

        fpr, tpr, _ = roc_curve(true_labels, mae_scores)
        roc_auc = auc(fpr, tpr)
        ...
        plt.title(f'AE MAE Anomaly Detection ROC - {data_split.capitalize()}')
        plt.legend(loc="lower right")
        plt.savefig(os.path.join(save_dir, f'roc_ae_mae_{data_split}.png'), bbox_inches='tight')
        plt.close()
    else:
...
def main(args):
    ...
    evaluate_ae_mae(valid_loader, encoder, decoder, device, args.save_dir, data_split="validation")
    evaluate_ae_mae(test_loader, encoder, decoder, device, args.save_dir, data_split="test")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Evaluate the Autoencoder trained with MAE")
    parser.add_argument('--model_path', type=str, required=True)
    parser.add_argument('--data_path', type=str, default='./dataset.h5')
    parser.add_argument('--save_dir', type=str, default='./results_ae_mae')
