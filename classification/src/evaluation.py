...
def evaluate_network(dataloader, model, loss_fn, device, data_split, save_dir, model_name, num_classes=5,
    ...
    with torch.no_grad(): # Remove gradient computation
        ...
        ax =sns.heatmap(cm, annot = True, fmt = '.2f', cmap = 'Blues', 
                    xticklabels = class_labels, yticklabels = class_labels, annot_kws={"size": 19})
        cbar = ax.collections[0].colorbar
        cbar.ax.tick_params(labelsize=19)
        plt.xlabel('Predicted Label', fontsize=19)
        plt.ylabel('True Label', fontsize=19)
        plt.title(f'Confusion Matrix - {data_split.capitalize()}', fontsize=21)
        ...
        plt.title(f'Multiclass ROC Curves - {data_split.capitalize()}',fontsize=21)
